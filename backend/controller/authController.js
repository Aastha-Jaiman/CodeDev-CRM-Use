...
exports.registerUser = async (req, res) => {
  ...
  try {
    // Extract saved OTP, name, and email from cookies
    ...
    // Ensure name and email exist in cookies
    if (!name || !email) {
      return res.status(400).send({
        success: false,
    ...
    if (!hashedPassword) {
      return res.status(500).send({
        success: false,
        message: "Failed to secure your password. Please try again later.",
      });
    }
    ...
    const userData = await user.create({
      ...
      userName: `user${Date.now()}`, // Ensure userName is unique
    });
    ...
    // Create default statuses for the user
    ...
    // Generate JWT token
    const token = jwt.sign({ id: userData._id }, process.env.JWT_KEY, { expiresIn: "1h" });
