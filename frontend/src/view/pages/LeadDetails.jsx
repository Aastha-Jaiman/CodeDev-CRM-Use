...
const LeadDetails = () => {
  ...
  const canReadLead = user?.user.role === "admin" || user?.permission?.lead?.read;
  const canUpdateLead = user?.user.role === "admin" || user?.permission?.lead?.update;
  const canDeleteLead = user?.user.role === "admin" || user?.permission?.lead?.delete;
  ...
  }, [user, canReadLead, id, navigate]);

  const fetchLeadDetails = async () => {
    try {
      setLoading(true);
      const data = await getLeadById(id);
      console.log('data in lead details', data)
      setLead(data);
    } catch (err) {
